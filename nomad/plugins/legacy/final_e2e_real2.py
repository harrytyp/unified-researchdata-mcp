...
print("=" * 60)
print("FINAL E2E TEST — Real Sample File (txt)")
print("=" * 60)
...
cr = requests.post(f"{ELAB_URL}/items", headers=elab_h,
    json={"title": f"Polymer-X TGA {ts}", "category": "5", "status": "67",
          "metadata": json.dumps({})}, verify=False, timeout=15)
item_id = int(cr.headers.get("Location","").split("/")[-1])
print(f"  Created item {item_id}")

# ── Step 2: Upload real txt to NOMAD ──
print("\n[2] Uploading real TGA txt to NOMAD...")
txt_path = "/app/plugins/test_watch/FinalDemo_item1458.txt"
filename = f"Polymer-X_item{item_id}.txt"
with open(txt_path, "rb") as f:
    txt_data = f.read()

r = requests.post(NOMAD_URL + "/uploads", headers=nomad_h,
    files={"file": (filename, txt_data, "text/plain")}, timeout=15)
