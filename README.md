...
## TGA Automation Pipeline

Automatic E2E pipeline for TA Instruments TGA measurements.

### Architecture

```
elabFTW (Team 29 - eConversion DataIntelligence)
  │
  ├── User creates TGA experiment from template (extra_fields form)
  ├── Status → "Running" triggers .tprc generation
  ├── Server watcher generates .tprc → attaches to elabFTW entry
  │
  ├── Windows App (.exe) downloads .tprc → TRIOS import folder
  ├── TRIOS runs measurement → exports .tri
  ├── Windows App uploads .tri as elabFTW attachment
  │
  └── Server watcher: detects .tri → parses → NOMAD → elabFTW body
```
...
| File | Purpose |
|------|---------|
| `tprc_builder.py` | Generate `.tprc` files from measurement parameters |
| `e2e_tga_service.py` | Core: .tri parser + NOMAD/elabFTW clients |
| `elab_watcher_service.py` | Monitors elabFTW for new TGA experiments |
| `tga_elab_app.pyw` | Windows tray app source |
| `samples/tga/TGA_elabFTW_Agent.exe` | Standalone Windows .exe (19MB) |

### Workflow
1. **elabFTW**: New experiment → fill form → status "Running"
2. **Windows App**: Select experiment → "Download .tprc"
3. **TRIOS**: Load method → run → export .tri
4. **Windows App**: Select experiment → "Upload .tri" (or auto-watcher)
5. **Server**: Parses → NOMAD → results in elabFTW body
