...
async def register_route(request: Request):
  ...
  <button onclick="(function(e){var b=document.querySelector('.url-box');if(!b||!b.textContent)return;var u=b.textContent.trim();var t=u.split('token=')[1];if(!t)return;t=t.split(/[\s<&]/)[0];navigator.clipboard.writeText(t).then(function(){var btn=e.currentTarget||e;btn.innerHTML='\u2713 Copied';setTimeout(function(){btn.innerHTML='<span style=font-size:1.2em>\U0001f4cb</span> Token'},2000)}).catch(function(){prompt('Token:',t)})})(this)" type="button" style="flex:1;padding:10px 14px;background:#1a2236;color:#e8edf5;border:1px solid #2a3f60;border-radius:10px;font-size:0.82rem;font-weight:600;cursor:pointer;font-family:inherit;display:flex;align-items:center;gap:8px;justify-content:center"><span style="font-size:1.2em">\U0001f4cb</span> Token</button>
  <button onclick="(function(e){var b=document.querySelector('.url-box');if(!b||!b.textContent)return;var u=b.textContent.trim();navigator.clipboard.writeText(u).then(function(){var btn=e.currentTarget||e;btn.innerHTML='\u2713 Copied';setTimeout(function(){btn.innerHTML='<span style=font-size:1.2em>\U0001f4c2</span> URL'},2000)}).catch(function(){prompt('URL:',u)})})(this)" type="button" style="flex:1;padding:10px 14px;background:#3b82f6;color:#fff;border:none;border-radius:10px;font-size:0.82rem;font-weight:600;cursor:pointer;font-family:inherit;display:flex;align-items:center;gap:8px;justify-content:center"><span style="font-size:1.2em">\U0001f4c2</span> URL</button>
</div>'''
