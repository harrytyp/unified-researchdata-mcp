...
# any lab's sample count, and 5 characters are what fits on a sample label. Any
...
def sample_id(upload_id: str | None) -> str:
    ...
    submitted, the requester writes it on the sample they send in, and the
    operator sees the same code on the board card and in the detail panel. The
    full upload_id stays authoritative everywhere (NOMAD, deep links).
